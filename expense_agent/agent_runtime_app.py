...
class AgentEngineApp(AdkApp):
    def set_up(self) -> None:
        ...
        if os.environ.get("INTEGRATION_TEST") == "TRUE":
            class MockLogger:
                def log_struct(self, data: Any, severity: str = "INFO") -> None:
                    logging.info(f"[MockLogger] {severity}: {data}")
            ...
            try:
                ...
                logging.warning(f"Failed to initialize Google Cloud Logging: {e}. Using fallback logger.")
                class FallbackLogger:
                    def log_struct(self, data: Any, severity: str = "INFO") -> None:
                        logging.info(f"[FallbackLogger] {severity}: {data}")
